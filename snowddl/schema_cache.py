...
class SnowDDLSchemaCache:
    ...
    def reload(self):
        ...
        for r in cur:
            ...
            if (
                r["owner"] != self.engine.context.current_role
                and not self.engine.settings.ignore_ownership
            ):
                continue
            ...
            if (
                self.engine.settings.include_databases
        ...
        for database_schemas in self.engine.executor.map(
            self._get_database_schemas, self.databases
        ):
