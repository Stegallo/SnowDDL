...
class SnowDDLContext:
    ...
    def _validate(self):
        ...
        if not self.is_account_admin:
            if self.engine.settings.execute_account_params:
                raise ValueError(
                    "Context error: missing ACCOUNTADMIN role in session to apply ACCOUNT PARAMETERS"
                )

            if self.engine.settings.execute_resource_monitor:
                raise ValueError(
                    "Context error: missing ACCOUNTADMIN role in session to apply RESOURCE MONITORS"
                )
    ...
    def activate_role_with_prefix(self):
        ...
        role_with_prefix = AccountObjectIdent(
            self.engine.config.env_prefix, self.current_role
        )

        cur = self.engine.execute_meta(
            "SHOW ROLES LIKE {role_with_prefix:lf}",
