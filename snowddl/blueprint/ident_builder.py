...
def build_schema_object_ident(
    env_prefix, object_name, context_database_name, context_schema_name
) -> SchemaObjectIdent:
    # Function or procedure identifier with arguments
    if object_name.endswith(")"):
        object_name, data_types_str = object_name.rstrip(")").split("(")
        data_types = (
            [BaseDataType[dt.strip(" ").upper()] for dt in data_types_str.split(",")]
            if data_types_str
            else []
        )
    else:
    ...
    if parts_len > 3:
        raise ValueError(
            f"Too many delimiters in schema object identifier [{object_name}]"
        )
    ...
    if isinstance(data_types, list):
        return SchemaObjectIdentWithArgs(
            env_prefix, parts[0], parts[1], parts[2], data_types=data_types
        )

    return SchemaObjectIdent(env_prefix, parts[0], parts[1], parts[2])


def build_role_ident(
    env_prefix, *args: Union[AccountObjectIdent, str]
) -> AccountObjectIdent:
    return AccountObjectIdent(
        env_prefix,
        f"{'__'.join(str(a.name) if isinstance(a, AccountObjectIdent) else str(a) for a in args)}",
    )
...
def build_grant_name_ident_snowflake(grant_name, object_type: ObjectType):
    ...
    if len(parts) == 3:
        ...
        if "(" in last_part:
            ...
            data_types = (
                [
                    BaseDataType[arg.strip(" ").split(" ")[1]]
                    for arg in data_types_str.split(",")
                ]
                if data_types_str
                else []
            )
    ...
    raise ValueError(
        f"Unexpected grant name format [{grant_name}] in Snowflake for object type [{object_type}]"
    )
