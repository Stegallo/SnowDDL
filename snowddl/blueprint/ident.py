    ...
    def _validate_part(self, val):
        ...
        for char in val:
            if char not in self.allowed_chars:
                raise ValueError(
                    f"Character [{char}] in not allowed in identifier [{val}], only "
                    "ASCII letters, digits and single underscores are accepted"
                )
...
class AbstractIdentWithPrefix(AbstractIdent, ABC):
    ...
    def _validate_env_prefix(self, val):
        ...
        for char in val:
            if char not in self.allowed_chars:
                raise ValueError(
                    f"Character [{char}] in not allowed in env prefix [{val}], only "
                    "ASCII letters, digits and single underscores are accepted"
                )
