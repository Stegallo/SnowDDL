...
class SnowDDLConfig:
    ...
    def __init__(self, env_prefix=None):
        ...
        self.blueprints: Dict[Type[T_Blueprint], Dict[str, T_Blueprint]] = defaultdict(
            dict
        )
    ...
    def get_blueprints_by_type_and_pattern(
        self, cls: Type[T_Blueprint], pattern: str
    ) -> Dict[str, T_Blueprint]:
        ...
        if any(special_char in pattern for special_char in ["*", "?", "[", "]"]):
            ...
            return {
                full_name: bp
                for full_name, bp in type_blueprints.items()
                if regexp.match(full_name)
            }
    ...
    def _init_env_prefix(self, env_prefix):
        if env_prefix:
            ...
            if "__" in env_prefix:
                raise ValueError(
                    f"Env prefix [{env_prefix}] cannot contain [__] double underscore"
                )

            if env_prefix.endswith("_"):
                raise ValueError(
                    f"Env prefix [{env_prefix}] cannot end with [_] underscore"
                )
