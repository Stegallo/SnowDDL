...
class UserParser(AbstractParser):
    def load_blueprints(self):
        self.parse_single_file(
            self.base_path / "user.yaml", user_json_schema, self.process_user
        )
    ...
    def process_user(self, f: ParsedFile):
        ...
        for user_name, user in f.params.items():
            ...
            for business_role_name in user["business_roles"]:
                business_roles.append(
                    build_role_ident(
                        self.env_prefix,
                        business_role_name,
                        self.config.BUSINESS_ROLE_SUFFIX,
                    )
                )
            ...
            bp = UserBlueprint(
                ...
                rsa_public_key=user.get("rsa_public_key").replace(" ", "")
                if user.get("rsa_public_key")
                else None,
                rsa_public_key_2=user.get("rsa_public_key_2").replace(" ", "")
                if user.get("rsa_public_key_2")
                else None,
                default_warehouse=AccountObjectIdent(self.env_prefix, default_warehouse)
                if default_warehouse
                else None,
                default_namespace=build_default_namespace_ident(
                    self.env_prefix, user.get("default_namespace")
                )
    ...
    def get_default_warehouse_map(self):
        ...
        business_role_config = self.parse_single_file(
            self.base_path / "business_role.yaml", business_role_json_schema
        )
        ...
        for business_role_name, business_role in business_role_config.items():
            ...
            default_warehouse_map[business_role_name] = business_role[
                "warehouse_usage"
            ][0]
