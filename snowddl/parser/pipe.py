...
class PipeParser(AbstractParser):
    ...
    def process_pipe(self, f: ParsedFile):
        ...
        bp = PipeBlueprint(
            ...
            copy_table_name=build_schema_object_ident(
                self.env_prefix, copy["table"], f.database, f.schema
            ),
            copy_stage_name=build_schema_object_ident(
                self.env_prefix, copy["stage"], f.database, f.schema
            ),
            copy_path=copy.get("path"),
            copy_pattern=copy.get("pattern"),
            copy_transform=self.normalise_params_dict(copy.get("transform")),
            copy_file_format=build_schema_object_ident(
                self.env_prefix, copy.get("file_format"), f.database, f.schema
            )
