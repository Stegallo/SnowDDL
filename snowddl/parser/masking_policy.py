...
class MaskingPolicyParser(AbstractParser):
    def load_blueprints(self):
        self.parse_schema_object_files(
            "masking_policy", masking_policy_json_schema, self.process_masking_policy
        )
    ...
    def process_masking_policy(self, f: ParsedFile):
        ...
        for a in f.params.get("references", []):
            ref = MaskingPolicyReference(
                ...
                object_name=build_schema_object_ident(
                    self.env_prefix, a["object_name"], f.database, f.schema
                ),
