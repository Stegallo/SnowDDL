...
stage_json_schema = {
    ...
    "properties": {
        ...
        "copy_options": {
            "type": "object",
            "additionalProperties": {"type": ["array", "boolean", "number", "string"]},
        },
        "comment": {"type": "string"},
    },
...
class StageParser(AbstractParser):
    ...
    def process_stage(self, f: ParsedFile):
        ...
        bp = StageBlueprint(
            ...
            storage_integration=Ident(f.params["storage_integration"])
            if f.params.get("storage_integration")
            else None,
            ...
            file_format=build_schema_object_ident(
                self.env_prefix, f.params["file_format"], f.database, f.schema
            )
            ...
            for path in stage_files_dir.glob("**/*"):
                ...
                bp = StageFileBlueprint(
                    full_name=StageFileIdent(
                        self.env_prefix, f.database, f.schema, f.name, path=stage_path
                    ),
                    local_path=str(path),
