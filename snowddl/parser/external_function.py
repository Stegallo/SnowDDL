...
class ExternalFunctionParser(AbstractParser):
    def load_blueprints(self):
        self.parse_schema_object_files(
            "external_function", external_function_json_schema, self.process_function
        )

    def process_function(self, f: ParsedFile):
        arguments = [
            NameWithType(name=Ident(k), type=DataType(t))
            for k, t in f.params.get("arguments", {}).items()
        ]
        ...
        bp = ExternalFunctionBlueprint(
            full_name=SchemaObjectIdentWithArgs(
                ...
                f.schema,
                base_name,
                [a.type.base_type for a in arguments],
            ),
            arguments=arguments,
            returns=returns,
            ...
            context_headers=[Ident(h) for h in f.params.get("context_headers")]
            if f.params.get("context_headers")
            else None,
            max_batch_rows=f.params.get("max_batch_rows"),
            compression=f.params.get("compression"),
            request_translator=build_schema_object_ident(
                self.env_prefix, f.params["request_translator"], f.database, f.schema
            )
            if f.params.get("request_translator")
            else None,
            response_translator=build_schema_object_ident(
                self.env_prefix, f.params["response_translator"], f.database, f.schema
            )
