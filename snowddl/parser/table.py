...
col_type_re = compile(
    r"^(?P<type>[a-z0-9_]+(\((\d+)(,(\d+))?\))?)" r"(?P<not_null> NOT NULL)?$",
    IGNORECASE,
)
...
table_json_schema = {
    ...
    "properties": {
        ...
        "search_optimization": {"type": "boolean"},
        "comment": {"type": "string"},
...
class TableParser(AbstractParser):
    ...
    def init_combined_params(self):
        ...
        for database_path in self.base_path.iterdir():
            ...
            database_params = self.parse_single_file(
                database_path / "params.yaml", database_json_schema
            )
            ...
            for schema_path in database_path.iterdir():
                ...
                schema_params = self.parse_single_file(
                    schema_path / "params.yaml", schema_json_schema
                )

                combined_params[database_path.name][schema_path.name] = {
                    "is_transient": database_params.get("is_transient", False)
                    or schema_params.get("is_transient", False),
    ...
    def process_table(self, f: ParsedFile):
        ...
        for col_name, col in f.params["columns"].items():
            ...
            if col.get("default_sequence"):
                col_default = build_schema_object_ident(
                    self.env_prefix, col["default_sequence"], f.database, f.schema
                )
            else:
        ...
        bp = TableBlueprint(
            ...
            is_transient=f.params.get(
                "is_transient",
                self.combined_params[f.database][f.schema].get("is_transient", False),
            ),
            retention_time=f.params.get(
                "retention_time",
                self.combined_params[f.database][f.schema].get("retention_time", None),
            ),
        ...
        for fk in f.params.get("foreign_keys", []):
            bp = ForeignKeyBlueprint(
                ...
                columns=[Ident(c) for c in fk["columns"]],
                ref_table_name=build_schema_object_ident(
                    self.env_prefix, fk["ref_table"], f.database, f.schema
                ),
