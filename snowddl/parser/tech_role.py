from snowddl.blueprint import Grant, ObjectType, TechRoleBlueprint, build_role_ident
...
class TechRoleParser(AbstractParser):
    def load_blueprints(self):
        self.parse_single_file(
            self.base_path / "tech_role.yaml",
            tech_role_json_schema,
            self.process_tech_role,
        )

    def process_tech_role(self, f: ParsedFile):
        for tech_role_name, tech_role in f.params.items():
            tech_role_ident = build_role_ident(
                self.env_prefix, tech_role_name, self.config.TECH_ROLE_SUFFIX
            )
