from snowddl.blueprint import Ident, ResourceMonitorBlueprint
...
class ResourceMonitorParser(AbstractParser):
    def load_blueprints(self):
        self.parse_single_file(
            self.base_path / "resource_monitor.yaml",
            resource_monitor_json_schema,
            self.process_resource_monitor,
        )
    ...
    def process_resource_monitor(self, f: ParsedFile):
        for name, monitor in f.params.items():
            bp = ResourceMonitorBlueprint(
                ...
                triggers={
                    int(k): str(v).upper() for k, v in monitor.get("triggers").items()
                },
                comment=None,
            )
