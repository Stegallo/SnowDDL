...
class FunctionParser(AbstractParser):
    def load_blueprints(self):
        self.parse_schema_object_files(
            "function", function_json_schema, self.process_function
        )
    ...
    def process_function(self, f: ParsedFile):
        ...
        if isinstance(f.params["returns"], dict):
            ...
            returns = [
                NameWithType(name=Ident(k), type=DataType(t))
                for k, t in f.params["returns"].items()
            ]
        else:
            # Returns value
        ...
        if f.params.get("imports"):
            imports = [
                StageWithPath(
                    stage_name=build_schema_object_ident(
                        self.env_prefix, i["stage"], f.database, f.schema
                    ),
                    path=i["path"],
                )
        ...
        bp = FunctionBlueprint(
            full_name=SchemaObjectIdentWithArgs(
                ...
                f.schema,
                base_name,
                data_types=[a.type.base_type for a in arguments],
            ),
