...
class TableConverter(AbstractSchemaObjectConverter):
    ...
    def dump_object(self, row):
        ...
        if row["cluster_by"]:
            data["cluster_by"] = cluster_by_syntax_re.sub(
                r"\2", row["cluster_by"]
            ).split(", ")
        ...
        if data:
            self._dump_file(
                object_path / f"{self._normalise_name(row['name'])}.yaml",
                data,
                table_json_schema,
            )
        ...
        for c in cur:
            ...
            if c["default"]:
                if str(c["default"]).upper().endswith(".NEXTVAL"):
                    col["default_sequence"] = self._normalise_name_with_prefix(
                        str(c["default"])[:-8]
                    )
                else:
    ...
    def _get_foreign_keys(self, row):
        ...
        for r in cur:
            ...
            constraints[r["fk_name"]]["columns"][r["key_sequence"]] = r[
                "fk_column_name"
            ]
            constraints[r["fk_name"]]["ref_columns"][r["key_sequence"]] = r[
                "pk_column_name"
            ]
