...
class ForeignKeyResolver(AbstractSchemaObjectResolver):
    ...
    def get_existing_objects_in_schema(self, schema: dict):
        ...
        for r in cur:
            if r["fk_name"] not in constraints_by_name:
                ...
                constraints_by_name[r["fk_name"]]["columns"][r["key_sequence"]] = r[
                    "fk_column_name"
                ]
                constraints_by_name[r["fk_name"]]["ref_columns"][r["key_sequence"]] = r[
                    "pk_column_name"
                ]
        ...
        for c in constraints_by_name.values():
            ...
            full_name = (
                f"{c['database']}.{c['schema']}.{c['table']}({','.join(columns_list)})"
            )
    ...
    def compare_object(self, bp: ForeignKeyBlueprint, row: dict):
        if (
            str(bp.ref_table_name) == row["ref_table_name"]
            and [str(c) for c in bp.ref_columns] == row["ref_columns"]
        ):
