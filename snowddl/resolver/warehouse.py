...
class WarehouseResolver(AbstractResolver):
    ...
    def get_existing_objects(self):
        ...
        for r in cur:
            ...
            existing_objects[r["name"]] = {
                ...
                "resource_monitor": r["resource_monitor"]
                if r["resource_monitor"] != "null"
                else None,
            }
    ...
    def _compare_properties(self, bp: WarehouseBlueprint, row: dict):
        if (
            self._normalise_warehouse_size(bp.size)
            == self._normalise_warehouse_size(row["size"])
            and bp.auto_suspend == row["auto_suspend"]
            and (
                row["min_cluster_count"] is None
                or coalesce(bp.min_cluster_count, 1) == row["min_cluster_count"]
            )
            and (
                row["max_cluster_count"] is None
                or coalesce(bp.max_cluster_count, 1) == row["max_cluster_count"]
            )
            and (
                row["scaling_policy"] is None
                or coalesce(bp.scaling_policy, "STANDARD") == row["scaling_policy"]
            )
        ):
            return False
    ...
    def _build_common_properties(self, bp: WarehouseBlueprint):
        ...
        if bp.min_cluster_count:
            query.append_nl(
                "MIN_CLUSTER_COUNT = {min_cluster_count:d}",
                {
        ...
        if bp.max_cluster_count:
            query.append_nl(
                "MAX_CLUSTER_COUNT = {max_cluster_count:d}",
                {
