...
class UserResolver(AbstractResolver):
    ...
    def compare_object(self, bp: UserBlueprint, row: dict):
        ...
        if self.engine.settings.refresh_user_passwords and self._refresh_password(
            bp, row
        ):
            result = ResolveResult.ALTER
    ...
    def _build_common_properties(self, bp: UserBlueprint):
        ...
        query.append_nl(
            "DISPLAY_NAME = {display_name}", {"display_name": bp.display_name}
        )
        ...
        query.append_nl(
            ...
            {"default_warehouse": bp.default_warehouse},
        )
        query.append_nl(
            "DEFAULT_NAMESPACE = {default_namespace}",
            {"default_namespace": bp.default_namespace},
        )
        query.append_nl(
            "DEFAULT_ROLE = {default_role:i}",
            {"default_role": self._get_user_role_ident(bp)},
        )
    ...
    def _compare_public_keys(self, bp: UserBlueprint, row: dict):
        if (
            not bp.rsa_public_key
            and not bp.rsa_public_key_2
            and not row["has_rsa_public_key"]
        ):
            return False
        ...
        if (
            bp.rsa_public_key == existing_public_key
    ...
    def _compare_parameters(self, bp: UserBlueprint):
        ...
        for param_name, param_value in bp.session_params.items():
            if param_name not in existing_params:
                raise ValueError(
                    f"Unknown parameter [{param_name}] for user [{bp.full_name}]"
                )

            if compare_dynamic_param_value(
                param_value, existing_params[param_name]["value"]
            ):
                continue
    ...
    def _get_user_role_ident(self, bp: UserBlueprint):
        return build_role_ident(
            self.config.env_prefix, bp.full_name, self.config.USER_ROLE_SUFFIX
        )
