...
from snowddl.resolver.abc_resolver import (
    AbstractResolver,
    ObjectType,
    ResolveResult,
    SnowDDLUnsupportedError,
)
...
class DatabaseResolver(AbstractResolver):
    ...
    def create_object(self, bp: DatabaseBlueprint):
        ...
        self.engine.execute_safe_ddl(
            ...
            {"database": bp.full_name, "schema": "PUBLIC"},
        )
    ...
    def compare_object(self, bp: DatabaseBlueprint, row: dict):
        if bp.is_transient != row["is_transient"]:
            if bp.is_transient:
                raise SnowDDLUnsupportedError(
                    "Cannot change PERMANENT object into TRANSIENT object"
                )
            else:
                raise SnowDDLUnsupportedError(
                    "Cannot change TRANSIENT object into PERMANENT object"
                )
    ...
    def drop_object(self, row: dict):
        self.engine.execute_unsafe_ddl(
            "DROP DATABASE {database:i}", {"database": row["database"]}
        )
