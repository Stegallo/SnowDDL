...
class ExternalFunctionResolver(AbstractSchemaObjectResolver):
    ...
    def get_existing_objects_in_schema(self, schema: dict):
        ...
        cur = self.engine.execute_meta(
            "SHOW EXTERNAL FUNCTIONS IN SCHEMA {database:i}.{schema:i}",
            {
    ...
    def _build_create_function(self, bp: ExternalFunctionBlueprint):
        ...
        query.append(
            "EXTERNAL FUNCTION {full_name:in} (",
            {
                "full_name": bp.full_name,
            },
        )
        ...
        if bp.headers:
            ...
            for idx, (header_name, header_value) in enumerate(bp.headers.items()):
                query.append_nl(
                    "    {comma:r}{header_name} = {header_value}",
                    {
                        "comma": "  " if idx == 0 else ", ",
        ...
        if bp.context_headers:
            query.append_nl(
                "CONTEXT_HEADERS = ({context_headers:r})",
                {
        ...
        if bp.max_batch_rows:
            query.append_nl(
                "MAX_BATCH_ROWS = {max_batch_rows:d}",
                {
        ...
        if bp.request_translator:
            query.append_nl(
                "REQUEST_TRANSLATOR = {request_translator:i}",
                {
        ...
        if bp.response_translator:
            query.append_nl(
                "RESPONSE_TRANSLATOR = {response_translator:i}",
                {
