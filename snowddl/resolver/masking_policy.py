...
class MaskingPolicyResolver(AbstractSchemaObjectResolver):
    ...
    def compare_object(self, bp: MaskingPolicyBlueprint, row: dict):
        ...
        if r[
            "signature"
        ] != f"({', '.join([f'{a.name} {a.type.base_type.name}' for a in bp.arguments])})" or r[
            "return_type"
        ] != str(
            bp.returns
        ):
    ...
    def drop_object(self, row: dict):
        self._drop_policy_refs(
            SchemaObjectIdent("", row["database"], row["schema"], row["name"])
        )
        self._drop_policy(
            SchemaObjectIdent("", row["database"], row["schema"], row["name"])
        )
    ...
    def _create_policy(self, bp: MaskingPolicyBlueprint):
        ...
        self.engine.execute_unsafe_ddl(
            query, condition=self.engine.settings.execute_masking_policy
        )

    def _drop_policy(self, policy: SchemaObjectIdent):
        self.engine.execute_unsafe_ddl(
            "DROP MASKING POLICY {full_name:i}",
            {"full_name": policy},
            condition=self.engine.settings.execute_masking_policy,
        )
    ...
    def _apply_policy_refs(self, bp: MaskingPolicyBlueprint, skip_existing=False):
        existing_policy_refs = (
            {} if skip_existing else self._get_existing_policy_refs(bp.full_name)
        )
