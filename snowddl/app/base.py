...
class BaseApp:
    ...
    def init_arguments_parser(self):
        ...
        parser.add_argument(
            "--show-sql",
            help="Show executed DDL queries",
            default=False,
            action="store_true",
        )
        ...
        parser.add_argument(
            "--placeholder-path",
            help="Path to config file with environment-specific placeholders",
            default=None,
            metavar="",
        )
        parser.add_argument(
            "--placeholder-values",
            help="Environment-specific placeholder values in JSON format",
            default=None,
            metavar="",
        )
        ...
        parser.add_argument(
            ...
            help="Comma-separated list of object types NOT to resolve",
            default=None,
            metavar="",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help="Additionally apply changes to MASKING POLICIES",
            default=False,
            action="store_true",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--apply-account-params",
            help="Additionally apply changes to ACCOUNT PARAMETERS",
            default=False,
            action="store_true",
        )
        parser.add_argument(
            "--apply-network-policy",
            help="Additionally apply changes to NETWORK POLICIES",
            default=False,
            action="store_true",
        )
        parser.add_argument(
            "--apply-resource-monitor",
            help="Additionally apply changes to RESOURCE MONITORS",
            default=False,
            action="store_true",
        )
        ...
        parser.add_argument(
            "--apply-outbound-share",
            help="Additionally apply changes to OUTBOUND SHARES",
            default=False,
            action="store_true",
        )
        ...
        parser.add_argument(
            "--refresh-user-passwords",
            help="Additionally refresh passwords of users",
            default=False,
            action="store_true",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help="Allow {destroy} action without --env-prefix",
            default=False,
            action="store_true",
        )
        ...
        subparsers.add_parser(
            "plan", help="Resolve objects, apply nothing, display suggested changes"
        )
        subparsers.add_parser(
            "apply",
            help="Resolve objects, apply safe changes, display suggested unsafe changes",
        )
        subparsers.add_parser(
            "destroy",
            help="Drop objects with specified --env-prefix, use it to reset dev and test environments",
        )
    ...
    def get_connection(self):
        ...
        if self.args.get("k"):
            ...
            key_password = (
                str(self.args.get("passphrase")).encode("utf-8")
                if self.args.get("passphrase")
                else None
            )
    ...
    def execute(self):
        ...
        with self.engine:
            ...
            if self.args.get("action") == "destroy":
                if not self.args.get("env_prefix") and not self.args.get(
    ...
    def output_engine_context(self):
        ...
        self.logger.info(
            f"Session = {self.engine.context.current_session}, User = {self.engine.context.current_user}"
        )
        self.logger.info(
            f"Role = {self.engine.context.current_role}, Warehouse = {self.engine.context.current_warehouse}"
        )
    ...
    def get_placeholder_path(self):
        if self.args.get("placeholder_path"):
            ...
            if not placeholder_path.is_file():
                raise ValueError(
                    f"Placeholder path [{self.args.get('placeholder_path')}] does not exist or not a file"
                )
    ...
    def get_placeholder_values(self):
        ...
        try:
            ...
            raise ValueError(
                f"Placeholder values [{self.args.get('placeholder_values')}] are not a valid JSON"
            )

        if not isinstance(placeholder_values, dict):
            raise ValueError(
                f"Placeholder values [{self.args.get('placeholder_values')}] are not JSON encoded dict"
            )
    ...
    def output_config_errors(self, config):
        for e in config.errors:
            self.logger.warning(
                f"[{e['path']}]: {''.join(TracebackException.from_exception(e['error']).format())}"
            )
