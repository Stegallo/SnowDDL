...
class SingleDbApp(BaseApp):
    ...
    def init_arguments_parser(self):
        ...
        parser.add_argument(
            "--target-db",
            help="Target database name in Snowflake account (default: same as --config-db)",
            default=None,
        )
    ...
    def init_config(self):
        ...
        if self.args.get("config_db"):
            self.config_db = DatabaseIdent(
                config.env_prefix, self.args.get("config_db")
            )

            if str(self.config_db) not in config.get_blueprints_by_type(
                DatabaseBlueprint
            ):
                raise ValueError(
        ...
        if self.args.get("target_db"):
            self.target_db = DatabaseIdent(
                config.env_prefix, self.args.get("target_db")
            )
        else:
    ...
    def convert_config(self, original_config: SnowDDLConfig):
        ...
        for bp_dict in original_config.blueprints.values():
            for bp in bp_dict.values():
                if (
                    isinstance(bp, (SchemaBlueprint, SchemaObjectBlueprint))
                    and bp.full_name.database_full_name == self.config_db
                ):
