...
class Helper:
    ...
    def show_unique_keys(self, database, schema, name):
        cur = self.execute(
            "SHOW UNIQUE KEYS IN TABLE {table_name:i}",
            {"table_name": SchemaObjectIdent(self.env_prefix, database, schema, name)},
        )

        sorted_cur = sorted(
            cur, key=lambda r: (r["constraint_name"], r["key_sequence"])
        )

        return [
            [r["column_name"] for r in g]
            for k, g in groupby(sorted_cur, lambda r: r["constraint_name"])
        ]
    ...
    def _init_env_prefix(self):
        return (
            environ.get("SNOWFLAKE_ENV_PREFIX", self.DEFAULT_ENV_PREFIX).upper() + "__"
        )

    def _init_edition(self):
        cur = self.execute(
            "SELECT SYSTEM$BOOTSTRAP_DATA_REQUEST('ACCOUNT') AS bootstrap_account"
        )
        r = cur.fetchone()
